...
import VoiceCallComponent from './VoiceCall.jsx';
import { getAllUsers } from '../utils/userHandler.js';
...
const Chat = ({  connected }) => {
  ...
  useEffect(() => {
    ...
    const getAndSetConversations = async () => {
  ...
  useEffect(() => {
    ...
    if (currentConversation) {
      setMessages([
        ...
        { id: 4, sender: { id: 1, name: 'You' }, text: 'testing?', timestamp: new Date(Date.now() - 3500000) },
        { id: 5, sender: { id: 2, name: 'John Doe' }, type: 'image', media: '/api/placeholder/400/300', text: 'Check out this photo!', timestamp: new Date(Date.now() - 3000000) },
      ]);
    }
  }, [currentConversation]);
  ...
  //  console.log(user)
    ...
    const message = {
      id: Date.now(),
      sender: { id: user._id, name: user.fullname , connectionId : socket.id },
      text: newMessage,
      timestamp: new Date(),
      receiver  : { id: currentConversation.id, name: currentConversation.name , email : currentConversation.email },
    };
    // setConversationId({ me : socket.id, other : currentConversation.id, meId : user._id, otherId : currentConversation.id });
    // setConversationId(socket.id);
    ...
    >
      ...
      >
        ...
        <div className="p-4 border-t border-gray-200 dark:border-blue-700 flex justify-center space-x-4">
          ...
          {zoomedMedia.sender.id === user._id && (
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
